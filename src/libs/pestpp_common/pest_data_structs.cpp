...
void PestppOptions::parse_plusplus_line(const string &line)
{
	string key;
	string value;
	...
	for (std::sregex_iterator i(tmp_line.begin(), tmp_line.end(), lambda_reg); i != end_reg; ++i)
	{
		...
		PPARG_STATUS stat = assign_plusplus_value_by_name(key, org_value, line);
		if (stat != PPARG_STATUS::ACCEPTED_ARG)
			throw PestParsingError("error parsing key, value" + key + "," + value + " on line:" + line);

	}
}


PestppOptions::PPARG_STATUS PestppOptions::assign_plusplus_value_by_name(string key, string org_value, string line)
{
	upper_ip(key);
	...
	if (value.size() > 0)
		if (passed_args.find(key) != passed_args.end())
		{
			...
			cout << "ERROR: Duplicate key word \"" + key + "\", possibly through an alias on line: " << line << endl;
			return PPARG_STATUS::DUPLICATE_ARG;
		}
	...
	if (key=="MAX_N_SUPER"){
		...
		if (value == "PROPACK")
			...
			cout << "ERROR: Invalid ++svd_pack: " << value << endl;
			return PPARG_STATUS::INVALID_ARG;
	...
	else if (key == "REG_FRAC")
	{
		...
		cout << "ERROR: '++reg_frac' has been deprecated - please use * regularization and PHIMLIM" << endl;
		return PPARG_STATUS::INVALID_ARG;
	}
	...
	else if (key == "UPGRADE_BOUNDS")
	{
		...
		else
		{
			...
			cout << "ERROR: unrecognozed 'upgrade_bounds' option: should 'robust' or 'cheap'" << endl;
			PPARG_STATUS::INVALID_ARG;
		}
	...
	else if (key == "GLOBAL_OPT")
	{
		...
		{
			cout << "ERROR: unrecognized global_opt arg: " << value;
			return PestppOptions::PPARG_STATUS::INVALID_ARG;
		}
	}
	else if (key == "DE_F")
	{
	...
	else if (key == "OPT_DIRECTION")
	{
		...
		else
		{
			...
			cout << "ERROR: ++opt_direction arg must be in {MAX,MIN}, not " << v << endl;
			return PPARG_STATUS::INVALID_ARG;
		}
	...
	else
	{
		...
		return PPARG_STATUS::NOTFOUND_ARG;
	}

	passed_args.insert(key);
	arg_map[key] = value;
	
	return PPARG_STATUS::ACCEPTED_ARG;
	
}

void PestppOptions::set_defaults()
{
	set<string>::iterator end = passed_args.end();
	if (passed_args.find("N_ITER_BASE") == end)
		n_iter_base = 100000000;
	if (passed_args.find("N_ITER_SUPER") == end)
		n_iter_super = 0;
	if (passed_args.find("MAX_N_SUPER") == end)
		max_n_super = 1000000000;
	if (passed_args.find("SUPER_EIGTHRES") == end)
		super_eigthres = 1.0e-6;
	if (passed_args.find("SVD_PACK") == end)
		svd_pack = PestppOptions::SVD_PACK::REDSVD;
	if (passed_args.find("MAT_INV") == end)
		mat_inv = PestppOptions::MAT_INV::JTQJ;

	auto_norm = -999;
	if (passed_args.find("SUPER_RELPARMAX") == end)
		super_relparmax = 0.1;
	if (passed_args.find("MAX_RUN_FAIL") == end)
		max_run_fail = 3;
	if (passed_args.find("ITERATION_SUMMARY") == end)
		iter_summary_flag = true;
	if (passed_args.find("DER_FORGIVE") == end)
		der_forgive = true;
	if (passed_args.find("OVERDUE_RESCHED_FAC") == end)
		overdue_reched_fac = 1.15;
	if (passed_args.find("OVERDUE_GIVEUO_FAC") == end)
		overdue_giveup_fac = 100;
	if (passed_args.find("REG_FRAC") == end)
		reg_frac = -1.0;
	if (passed_args.find("GLOBAL_OPT") == end)
		global_opt = PestppOptions::GLOBAL_OPT::NONE;
	if (passed_args.find("DE_F") == end)
		de_f = 0.8;
	if (passed_args.find("DE_CR") == end)
		de_cr = 0.9;
	if (passed_args.find("DE_NPOPULATION") == end)
		de_npopulation = 40;
	if (passed_args.find("DE_MAX_GEN") == end)
		de_max_gen = 100;
	if (passed_args.find("DE_DITHER_F") == end)
		de_dither_f = true;
	if (passed_args.find("LAMBDAS") == end)
		base_lambda_vec = { 0.1, 1.0, 10.0, 100.0, 1000.0 };
	if (passed_args.find("LAMBDA_SCALE_FAC") == end)
		lambda_scale_vec = { 1.0 };
	if (passed_args.find("MAX_REG_ITER") == end)
		max_reg_iter = 50;
